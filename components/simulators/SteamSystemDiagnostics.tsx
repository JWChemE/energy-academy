...
export default function SteamSystemDiagnostics() {
  ...
  const theCase = caseId ? getSteamCase(caseId) : null;

  const score = useMemo(() => {
    if (!theCase) return null;
    return scoreSteamCase(theCase, calcStatuses, [...causes], plan);
  }, [theCase, calcStatuses, causes, plan]);
  ...
  const calcResolved =
    theCase && calcStatuses.length === theCase.calcParts.length &&
    ...
    return (
      <div>
        <p className="mb-4 text-sm text-slate-600">
          ...
          temperatures, pressures, dimensions — and you must <strong>calculate</strong> the figure that
          matters (condensate return, flash %, trap loss, heat loss…) before you can diagnose and fix it.
          Reference data is on hand; the method is yours to recall.
        </p>
  ...
  const causeItems = theCase.candidateCauseIds.map((id) => STEAM_CAUSES.find((x) => x.id === id)!);
  const actionItems = theCase.candidateActionIds.map((id) => STEAM_ACTIONS.find((x) => x.id === id)!);
          ...
          <div className="space-y-4">
            ...
            {!calcResolved && (
              <p className="text-right text-xs text-slate-400">
                Solve or skip each calculation to continue.
              </p>
            )}
          </div>
        </div>
          ...
          <div className="space-y-4">
            <div className="rounded-xl border border-slate-200 bg-white p-4">
              ...
              <p className="mb-3 text-xs text-slate-500">
                Use your calculated figures and the evidence. Select every root cause that applies — and only those.
              </p>
              <Checklist items={causeItems} selected={causes} onToggle={toggleCause} />
            </div>
            <div className="flex justify-between">
              <button
                onClick={() => setStep("calculate")}
        ...
        <div className="space-y-5">
          ...
          <div className="grid gap-5 lg:grid-cols-2">
            ...
            <div className="rounded-xl border border-slate-200 bg-white p-4">
              ...
              <AnswerReview items={causeItems} selected={causes} correct={new Set(theCase.correctCauseIds)} />
                  ...
                  <ul className="space-y-1">
                    {score.improvementsSpotted.map((id) => (
                      <li key={id} className="text-xs text-indigo-900">
                        ✓ {STEAM_ACTIONS.find((a) => a.id === id)?.label}
                      </li>
                    ))}
                  </ul>
...
function Readings({ c }: { c: (typeof STEAM_CASES)[number] }) {
  return (
        ...
        return (
          <button
            ...
            className={`flex w-full items-start gap-2.5 rounded-lg border p-2.5 text-left text-sm transition ${
              on ? "border-teal-400 bg-teal-50" : "border-slate-200 bg-white hover:border-slate-300"
            }`}
          >
            <span
