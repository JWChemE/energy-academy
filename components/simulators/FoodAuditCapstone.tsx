...
import { STAGES, DATASET, REFERENCE } from "@/lib/foodAuditCapstone";

/** Ashdown Foods — the on-site data stage's table + reference panel. */
function DataPanel() {
  return (
    <div className="mt-4 space-y-3">
          ...
          <tbody className="divide-y divide-slate-100">
            {DATASET.map((r) => (
              <tr key={r.month} className="text-slate-700">
                ...
                <td className="px-2 py-1.5">{r.tonnes.toLocaleString()}</td>
      ...
      <div className="flex flex-wrap gap-x-4 gap-y-1 rounded-lg bg-slate-50 px-3 py-2 text-xs text-slate-600">
        ...
        <span>Annual production {REFERENCE.annualTonnes.toLocaleString()} t</span>
      </div>
    </div>
  );
}

export default function FoodAuditCapstone() {
